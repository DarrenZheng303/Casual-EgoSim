#!/usr/bin/env bash
# EgoSim chunkwise Stage 2.
set -euo pipefail
source "$(dirname "${BASH_SOURCE[0]}")/egosim_train_common.sh"

CONFIG_PATH="${CONFIG_PATH:-${ROOT_DIR}/configs/causal_cd_chunkwise_egosim.yaml}"
RUN_TIMESTAMP="${RUN_TIMESTAMP:-$(date +%Y%m%d_%H%M%S)}"
LOGDIR="${LOGDIR:-${ROOT_DIR}/checkpoints/egosim_stage2_cd_chunkwise}"
RUN_OUTPUT_DIR="${RUN_OUTPUT_DIR:-${ROOT_DIR}/output/egosim_stage2_cd_chunkwise_${RUN_TIMESTAMP}}"
...
STAGE1_CKPT="${STAGE1_CKPT:-${ROOT_DIR}/checkpoints/egosim_stage1_chunkwise/checkpoint_model_001000/model.pt}"
MAX_TRAIN_STEPS="${MAX_TRAIN_STEPS:-1000}"
SAVE_EVAL_INTERVAL="${SAVE_EVAL_INTERVAL:-500}"
MASTER_PORT="${MASTER_PORT:-6062}"
RDZV_ID="${RDZV_ID:-${MA_JOB_NAME:-egosim_stage2_cd_chunkwise}}"
...
{
  echo "===== EgoSim Stage-2 Causal Forcing++ Chunkwise CCD ====="
  echo "LOGDIR=${LOGDIR}"
