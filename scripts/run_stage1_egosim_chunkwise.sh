#!/usr/bin/env bash
# EgoSim chunkwise Stage 1.
set -euo pipefail
source "$(dirname "${BASH_SOURCE[0]}")/egosim_train_common.sh"

CONFIG_PATH="${CONFIG_PATH:-${ROOT_DIR}/configs/ar_diffusion_tf_chunkwise_egosim.yaml}"
RUN_TIMESTAMP="${RUN_TIMESTAMP:-$(date +%Y%m%d_%H%M%S)}"
LOGDIR="${LOGDIR:-${ROOT_DIR}/checkpoints/egosim_stage1_chunkwise}"
RUN_OUTPUT_DIR="${RUN_OUTPUT_DIR:-${ROOT_DIR}/output/egosim_stage1_chunkwise_${RUN_TIMESTAMP}}"
...
RDZV_ID="${RDZV_ID:-${MA_JOB_NAME:-egosim_stage1_chunkwise}}"
...
{
  echo "===== EgoSim Stage-1 Causal Forcing Chunkwise Teacher Forcing ====="
  echo "LOGDIR=${LOGDIR}"
  echo "CONFIG_PATH=${RUNTIME_CONFIG}"
  echo "EGOSIM_MODEL_ROOT=${EGOSIM_MODEL_ROOT}"
  echo "EGOSIM_DATA_PATH=${EGOSIM_DATA_PATH}"
