...
Object-DMD applies the existing DMD gradient with weights derived from the
Grounded-SAM/CoTracker region. The selected region keeps weight `1.0`, the
background uses `0.1`, and every frame is normalized to mean weight `1.0`.
Training fails if any sample is missing valid physics data or has an empty
CoTracker region.

Build the EgoDex cache with `scripts/cache_egodex_train_modelart.py`, then use
the matching training chain:

```bash
# Chunkwise causal: Stage 1 -> Stage 2 -> Stage 3
bash scripts/run_stage1_egosim_chunkwise.sh
bash scripts/run_stage2_egosim_chunkwise.sh
bash scripts/run_stage3_egosim_chunkwise_object_dmd.sh

# Framewise causal: Stage 1 -> Stage 2 -> Stage 3
bash scripts/run_stage1_egosim_framewise.sh
bash scripts/run_stage2_egosim_framewise.sh
bash scripts/run_stage3_egosim_framewise_object_dmd.sh

# Bidirectional student initialized from the original EgoSim weights
bash scripts/run_stage3_egosim_bid_object_dmd.sh
```
