...
class _ImageEncoderModel(nn.Module):
...
class WanImageEncoder(nn.Module):
    ...
    def encode_image(self, videos):
        ...
        if isinstance(videos, (list, tuple)):
            videos = torch.cat([
                F.interpolate(
                    video if video.dim() == 4 else video.unsqueeze(0),
                    size=size,
                    mode='bicubic',
                    align_corners=False,
                )
        ...
        videos = self.normalize(videos.mul(0.5).add(0.5))
        parameter = next(self.model.visual.parameters())
        videos = videos.to(device=parameter.device, dtype=parameter.dtype)
