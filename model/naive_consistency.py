...
class NaiveConsistency(BaseModel):
    def __init__(self, args, device):
        ...
        self.model_type = getattr(args, "model_type", "wan")
    ...
    def _initialize_models(self, args, device):
        self.model_type = getattr(args, "model_type", "wan")
        if self.model_type == "egosim":
            load_pretrained_weights = not bool(getattr(args, "generator_ckpt", False))
            init_on_meta = bool(getattr(args, "generator_ckpt", False))
            ...
            self.teacher = EgoSimDiffusionWrapper(
                model_root=args.egosim_model_root,
                timestep_shift=getattr(args, "timestep_shift", 5.0),
                local_attn_size=getattr(args, "local_attn_size", -1),
                sink_size=getattr(args, "sink_size", 0),
            ...
            self.generator_ema = EgoSimDiffusionWrapper(
                model_root=args.egosim_model_root,
                timestep_shift=getattr(args, "timestep_shift", 5.0),
                local_attn_size=getattr(args, "local_attn_size", -1),
                sink_size=getattr(args, "sink_size", 0),
